 ...
 * steps DualScreenManager takes (a regression guard for upstream's 59efbbbc): apply the layout, flip the live arrangement, store the screen now
...
class RoleSwapPersistenceTest {
    ...
    private fun start(stored: ScreenLayout): Live {
        ...
        return Live(primary to presentation, primary == TOP, stored)
    }
    ...
    private fun Live.swap(): Live {
        ...
        return next.copy(stored = stored.withRole(keysByDisplayId.getValue(shownPrimary), ScreenRole.PRIMARY))
    }
