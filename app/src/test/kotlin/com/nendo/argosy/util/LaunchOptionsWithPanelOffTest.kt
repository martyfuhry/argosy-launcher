...
class LaunchOptionsWithPanelOffTest {
    ...
    private fun helper(bottomState: Int): DisplayAffinityHelper {
        ...
        return DisplayAffinityHelper(context, mockk(relaxed = true)).apply { dualScreenEnabled = true }
    }
    ...
    @Test
    fun `an unswapped single-screen game takes the top panel while the bottom one is off`() {
        val affinity = helper(bottomState = Display.STATE_OFF)

        val displayId = affinity.gameDisplayId(drawsSecondScreen = false, explicitDisplayId = null, rolesSwapped = false)
